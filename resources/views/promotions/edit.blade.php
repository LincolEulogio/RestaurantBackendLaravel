<x-app-layout>
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Promoción</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Actualiza los detalles de esta oferta</p>
            </div>
            ...
            <form action="{{ route('promotions.update', $promotion) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2 md:col-span-1">
                        ...
                        <select name="badge_label" id="badge_label"
                            ...
                            <option value="Popular"
                                {{ old('badge_label', $promotion->badge_label) == 'Popular' ? 'selected' : '' }}>Popular
                            </option>
                            <option value="Nuevo"
                                {{ old('badge_label', $promotion->badge_label) == 'Nuevo' ? 'selected' : '' }}>Nuevo
                            </option>
                            <option value="Limitado"
                                {{ old('badge_label', $promotion->badge_label) == 'Limitado' ? 'selected' : '' }}>
                                Limitado</option>
                            <option value="Oferta"
                                {{ old('badge_label', $promotion->badge_label) == 'Oferta' ? 'selected' : '' }}>Oferta
                            </option>
                        </select>
                        @error('badge_label')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" required
                            class="w-full rounded-xl border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:border-blue-500 focus:ring-blue-500 dark:text-white"
                            placeholder="Detalla las condiciones de la promoción...">{{ old('description', $promotion->description) }}</textarea>
                        @error('description')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_percent" id="discount_percent"
                            value="{{ old('discount_percent', $promotion->discount_percent) }}" min="0"
                            max="100"
                            class="w-full rounded-xl border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:border-blue-500 focus:ring-blue-500 dark:text-white"
                            placeholder="Ej: 50">
                    ...
                    <div class="col-span-2">
                        ...
                        @if ($promotion->image)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $promotion->image) }}" alt="Imagen actual"
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        // Pre-selected products for this promotion
                        const selectedProducts = @json($promotion->products->pluck('id'));
                        ...
                        function createRow(selectedId = null) {
                            ...
                            products.forEach(p => {
                                ...
                                options +=
                                    `<option value="${p.id}" ${isSelected}>${p.name} - S/ ${parseFloat(p.price).toFixed(2)}</option>`;
                            });
                        ...
                        if (selectedProducts.length > 0) {
                            selectedProducts.forEach(id => createRow(id));
                        } else {
                            createRow();
                        }
                    });
